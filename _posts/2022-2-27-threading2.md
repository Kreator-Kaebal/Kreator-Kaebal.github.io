...
가장 간단한 방법으로는 한 쓰레드가 임계 영역에 들어가는 동안, 다른 쓰레드가 **임계 영역에 못 들어가게 막아**버리면 된다.  이를 파이썬에서 활용하는 방법 중 하나로 threading 라이브러리에서 제공하는 **lock**이 있다.
...
이상적으론 서로 Lock을 빠져나간 뒤 다른 Lock에 들어가면 되지만, 이 순서가 꼬여 서로 Lock을 빠져나가지 못한 채 무한 대기하는 상황이 발생해버린다. 이를 교착 상태라고 한다.  
