...
당연하게 보안상 매우 큰 취약점이다. 이를 해결하는 방법이 하나 있다.
그 전에 알아두어야 할 것이...

### 동적 라우팅

next.js에서는 **변수로 주소를 정하는** 동적 라우팅이란 기능을 제공한다. 사실 next.js의 원류인 리액트에서부터 제공하는 기능이다.
예를 들어 내 아이디는 asdf이고, 다른 사람 아이디는 ㅁㄴㅇㄹ인데 아이디를 변수로 지정하면 /asdf, /ㅁㄴㅇㄹ 식으로 
