...
- 컨테이너 안에서 배포하기 위한 패키지(podman, fuse-overlayfs등) 설치
  - 사실 ```Docker Tools```로 호스트(VM)에 깔린 podman을 사용해도 되겠지만, 루트리스 관련 오류 땜에 컨테이너가 무거워지더라도 그냥 내부에서 설치 하였다.
...
이렇게 수정한 다음 빌드하고(당직은 ```sudo podman build -t my-jenkins2 .``` 이렇게 my-jenkins**2**로 했다)
...
아무튼 당직은 이 구조를 ```Backend_Manifests```이라는 이름으로 레포지토리화했다.
