---
layout: post
title: 프로젝트 좀보이드(Project Zomboid) 서버 고도화② - Openstack 단일 클러스터 구축(사전 작업, keystone 설치)
categories: [네트워크/인프라]
tags: [linux, openstack]
excerpt: Openstack 클러스터 구축기1
---
...
굳이 표준인 22번을 쓰지 않고 10022번으로 접속하는 이유는 무차별 로그인 시도  
(브루트-포스 brute-force, 여론조사 전화처럼 무작위 IP로 접속 시도를 하는 것)를 줄이기 위해서이다.
